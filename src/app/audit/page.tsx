  ...
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-900/70">
      <div className="border-b border-zinc-800 px-5 py-4">
        <h2 className="text-lg font-semibold text-white">Eventos do sistema</h2>
        <p className="text-sm text-zinc-500">Ultimos {rows.length} registros de escrita no tenant.</p>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-zinc-800 text-sm">
          <thead className="bg-zinc-950/60 text-left text-xs uppercase tracking-wide text-zinc-500">
            <tr>
          ...
          <tbody className="divide-y divide-zinc-800">
            {rows.map((row) => (
              <tr key={row.id} className="align-top">
                <td className="whitespace-nowrap px-5 py-3 text-zinc-400">{formatDate(row.created_at)}</td>
                <td className="px-5 py-3 font-medium text-white">{row.action}</td>
        ...
        <table className="min-w-full divide-y divide-zinc-800 text-sm">
          ...
          <tbody className="divide-y divide-zinc-800">
            {rows.map((row) => (
              <tr key={row.id} className="align-top">
                <td className="whitespace-nowrap px-5 py-3 text-zinc-400">{formatDate(row.created_at)}</td>
                <td className="px-5 py-3 font-medium text-white">{row.provider}</td>
                <td className="px-5 py-3 text-zinc-400">
                ...
                <td className="max-w-sm px-5 py-3 text-zinc-400">{row.message ?? "-"}</td>
                <td className="px-5 py-3">{renderMetadata(row.metadata)}</td>
              </tr>
            ))}
            {rows.length === 0 ? (
...
function renderMetadata(metadata: Record<string, unknown> | null) {
  ...
  return (
    <details className="max-w-sm">
      <summary className="cursor-pointer text-amber-400 hover:underline">Ver dados</summary>
...
function statusClassName(status: string) {
  ...
  if (status === "success") return `${base} bg-emerald-400/10 text-emerald-300`;
  if (status === "error") return `${base} bg-red-400/10 text-red-300`;
