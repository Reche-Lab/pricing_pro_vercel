...
const bodySchema = z.object({
  diameterMm: z.number().min(10).max(300).optional(), scale: z.number().min(1).max(5).default(1),
...
export async function POST(request: Request, route: { params: Promise<{ token: string; itemId: string; artworkId: string }> }) {
  ...
  const diameterMm = body.data.diameterMm ?? context.diameterMm;
  if (!diameterMm) return NextResponse.json({ ok: false, error: "O produto não possui diâmetro configurado." }, { status: 422 });
  try {
    const source = await loadArtworkDataUrl(context.artwork.data_url, context.artwork.storage_path);
    const prepared = await prepareCircularArtwork({ dataUrl: source, diameterMm, bleedMm: context.profile.bleedMm, dpi: context.profile.dpi, scale: body.data.scale, offsetX: body.data.offsetX, offsetY: body.data.offsetY, rotationDegrees: body.data.rotationDegrees });
    ...
    await savePublicPreparedArtwork({ context, artworkId: params.data.artworkId, diameterMm, prepared, preparedDataUrl: storagePath ? null : prepared.dataUrl, preparedStoragePath: storagePath, crop: body.data });
