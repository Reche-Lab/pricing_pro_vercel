...
import { deleteArtworkObject, loadArtworkDataUrl, uploadArtworkObject } from "@/services/storage/artwork-storage";
...
export async function POST(request: Request, route: { params: Promise<{ token: string; itemId: string }> }) {
  ...
  const limited = await enforcePublicRateLimit(request, params.data.token, {
    action: body.data.action === "generate" ? "artwork-ai-generate" : "artwork-ai-suggest",
    limit: body.data.action === "generate" ? 5 : 10,
    windowSeconds: 3600
  });
  ...
  try {
    ...
    uploadedStoragePath = await uploadArtworkObject({
      path: `${context.tenantId}/quotes/${context.quoteId}/items/${context.itemId}/original/${fileName}`,
      contentType: "image/webp",
      bytes
    });
    const artwork = await addPublicArtwork({ context, artworkName: "Versão criada pelo assistente", fileName, mimeType: "image/webp", fileSize: bytes.length, dataUrl, storagePath: uploadedStoragePath });
    ...
    await deleteArtworkObject(uploadedStoragePath);
