...
export async function GET(request: Request) {
  return withAgentAuthGet(request, "products:read", async (context, currentRequest) => {
    ...
    const limit = safeInteger(url.searchParams.get("limit"), 100, 1, 500);
    const offset = safeInteger(url.searchParams.get("offset"), 0, 0, 100_000);
    const category = url.searchParams.get("category");
    const result = await searchAgentProductMatches(context, "", { limit, offset, category });
    await logAgentAudit(context, "agent.products.list", {
      category,
      offset,
      limit,
