...
export type PublicArtworkContext = {
  ...
  artwork: {
    ...
    bleed_mm: string | null; safe_margin_mm: string | null;
  } | null;
};
...
export async function getPublicArtworkContext(token: string, itemId: string, artworkId?: string | null) {
  ...
  try {
    const result = await client.query<{
      ...
      print_bleed_mm: string | null; print_safe_margin_mm: string | null;
    }>(
    ...
    const [profileResult, artworkResult] = await Promise.all([
      client.query<{
        page_width_mm: string; page_height_mm: string; margin_mm: string; side_margin_mm: string; bottom_margin_mm: string; bleed_mm: string;
    ...
    return {
      ...
      geometry,
      margins: resolvePrintMargins({ ...row, ...artworkResult.rows[0] }, profile),
  ...
  try {
    const result = await client.query<{ id: string }>(
      ...
      [input.context.tenantId, input.context.quoteId, input.context.itemId, input.artworkId,
        ...
        input.margins.bleedMm, input.margins.safeMarginMm, input.context.profile.dpi,
...
function mapProfile(row: {
  ...
  return {
    ...
    marginMm: Number(row.margin_mm), sideMarginMm: Number(row.side_margin_mm ?? 4), bottomMarginMm: Number(row.bottom_margin_mm || 15), bleedMm: Number(row.bleed_mm), safeMarginMm: Number(row.safe_margin_mm),
