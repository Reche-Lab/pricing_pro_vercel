  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/settings/artwork-production", {
      method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({
        ...
        marginMm: Number(form.get("marginMm")), bleedMm: Number(form.get("bleedMm")),
    ...
    <details className="rounded-lg border border-zinc-800 bg-zinc-900/70">
      ...
      <form className="grid gap-4 border-t border-zinc-800 p-5" onSubmit={submit}>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Input defaultValue={profile.marginMm} label="Margem (mm)" name="marginMm" />
          <Input defaultValue={profile.gapMm} label="Espaço entre artes (mm)" name="gapMm" />
          <Input defaultValue={profile.bleedMm} label="Sangria (mm)" name="bleedMm" />
          <Input defaultValue={profile.safeMarginMm} label="Área segura (mm)" name="safeMarginMm" />
...
function Input({ defaultValue, label, name, step = "0.1" }: { defaultValue: number; label: string; name: string; step?: string }) {
  return <label><span className="mb-1 block text-xs font-medium text-zinc-400">{label}</span><input className="focus-ring w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm" defaultValue={defaultValue} min="0" name={name} required step={step} type="number" /></label>;
}
