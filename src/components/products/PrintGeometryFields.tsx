...
export function PrintGeometryFields({ defaults }: { defaults?: Partial<PrintGeometryFormValues> }) {
  ...
  return (
    <div className="mt-4 rounded-lg border border-cyan-900/60 bg-cyan-950/20 p-4">
      ...
      <p className="mt-1 text-xs leading-5 text-zinc-500">As dimensões definem a parte visível do produto até a sangria. O fundo deve continuar pela margem externa até o corte físico, enquanto a segurança protege os elementos importantes.</p>
      <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <GeometryInput defaultValue={defaults?.bleedMm ?? 2} label="Margem entre sangria e corte (mm)" min="0" name="printBleedMm" />
        <GeometryInput defaultValue={defaults?.safeMarginMm ?? 2} label="Margem interna de segurança (mm)" min="0" name="printSafeMarginMm" />
      </div>
      <div className="mt-3 grid gap-1 rounded-md border border-zinc-800 bg-zinc-950/60 px-3 py-2 text-[11px] leading-5 text-zinc-500"><span><strong className="text-cyan-300">Segurança:</strong> menor área; mantenha textos e elementos importantes dentro dela.</span><span><strong className="text-amber-300">Sangria:</strong> linha intermediária e limite da parte visível no produto.</span><span><strong className="text-rose-300">Corte efetivo:</strong> maior linha; continue o fundo da sangria até ela para evitar bordas brancas. Essa faixa ficará oculta.</span></div>
