...
import { Check, ChevronDown, ChevronUp, Download, Eye, ImageIcon, Printer, WandSparkles, X } from "lucide-react";
...
export function ArtworkProductionPanel({ quoteId, items }: { quoteId: string; items: QuoteItemRow[] }) {
  ...
  async function requestAi(action: "suggest" | "generate") {
    const item = items.find((entry) => entry.id === aiItemId) ?? items[0];
    const reference = artworks.find((entry) => entry.item.id === item?.id);
  ...
  return (
    <section className="border-t border-zinc-800 bg-zinc-950/30">
      ...
      {open ? <div className="grid gap-4 border-t border-zinc-800 p-4">
        ...
        {items.length ? <details className="rounded-md border border-violet-900/60 bg-violet-950/20 p-3"><summary className="cursor-pointer list-none text-sm font-medium text-violet-100"><span className="inline-flex items-center gap-2"><WandSparkles size={15} /> Assistente criativo via OpenRouter</span></summary><div className="mt-3 grid gap-3"><label><span className="mb-1 block text-xs font-medium text-zinc-400">Produto que receberá a arte</span><select className="focus-ring w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-white" value={aiItemId} onChange={(event) => setAiItemId(event.target.value)}>{items.map((item) => <option key={item.id} value={item.id}>{item.description} · {item.quantity} un.</option>)}</select></label><textarea className="focus-ring min-h-24 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-white" maxLength={3000} placeholder="Descreva tema, texto, cores, público e referências do cliente..." value={brief} onChange={(event) => setBrief(event.target.value)} /><div className="flex flex-wrap gap-2"><button className="focus-ring rounded-md border border-violet-700 px-3 py-2 text-xs font-medium text-violet-200 hover:bg-violet-950 disabled:opacity-50" disabled={Boolean(busy)} type="button" onClick={() => requestAi("suggest")}>Sugerir direção</button><button className="focus-ring rounded-md bg-violet-400 px-3 py-2 text-xs font-semibold text-violet-950 hover:bg-violet-300 disabled:opacity-50" disabled={Boolean(busy)} type="button" onClick={() => requestAi("generate")}>Gerar nova versão</button></div>{suggestions ? <SuggestionResult suggestions={suggestions} /> : null}</div></details> : null}
...
function ArtworkRow({ entry, quoteId, diameter, quantity, busy, onQuantity, onEdit, onApprove }: { entry: ArtworkEntry; quoteId: string; diameter: number | null; quantity: number; busy: string; onQuantity: (quantity: number) => void; onEdit: (entry: ArtworkEntry) => void; onApprove: (entry: ArtworkEntry, status: "approved" | "rejected") => void }) {
  ...
  return <div className="grid gap-3 rounded-md border border-zinc-800 bg-zinc-900/40 p-3 md:grid-cols-[64px_minmax(0,1fr)_150px_auto] md:items-center">
    {/* eslint-disable-next-line @next/next/no-img-element */}
    <img alt={entry.artwork.artwork_name ?? entry.artwork.file_name} className="h-16 w-16 rounded-full border border-zinc-700 object-cover" src={artworkImageUrl(quoteId, entry, prepared ? "prepared" : "original")} />
    <div className="min-w-0"><p className="truncate text-sm font-medium text-white">{entry.artwork.artwork_name || entry.artwork.file_name}</p><p className="text-xs text-zinc-500">{diameter ? `${diameter} mm` : "diâmetro pendente"} · {entry.artwork.source_kind === "openrouter" ? "gerada por IA" : "arquivo enviado"}</p><p className={`mt-1 text-xs ${entry.artwork.quality_status === "warning" ? "text-amber-300" : prepared ? "text-emerald-300" : "text-zinc-500"}`}>{entry.artwork.preparation_notes || "Aguardando enquadramento."}</p></div>
    <label><span className="mb-1 block text-xs text-zinc-500">Cópias desta arte</span><input className="focus-ring w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm" min="1" type="number" value={quantity} onChange={(event) => onQuantity(Math.max(1, Number(event.target.value) || 1))} /></label>
    <div className="flex flex-wrap gap-2 md:justify-end"><button className="focus-ring inline-flex h-9 items-center gap-2 rounded-md border border-zinc-700 px-3 text-xs text-zinc-200 hover:bg-zinc-900 disabled:opacity-50" disabled={!diameter || Boolean(busy)} type="button" onClick={() => onEdit(entry)}><ImageIcon size={14} /> {prepared ? "Reenquadrar" : "Enquadrar"}</button>{approved ? <><button className="focus-ring inline-flex h-9 items-center gap-2 rounded-md bg-emerald-400/15 px-3 text-xs font-medium text-emerald-200 disabled:opacity-50" disabled={Boolean(busy)} type="button" onClick={() => onApprove(entry, "approved")}><Check size={14} /> Atualizar</button><button className="focus-ring grid h-9 w-9 place-items-center rounded-md border border-zinc-700 text-zinc-400 hover:text-red-300" title="Remover aprovação" type="button" onClick={() => onApprove(entry, "rejected")}><X size={14} /></button></> : <button className="focus-ring inline-flex h-9 items-center gap-2 rounded-md bg-cyan-400 px-3 text-xs font-medium text-cyan-950 disabled:opacity-50" disabled={!prepared || Boolean(busy)} type="button" onClick={() => onApprove(entry, "approved")}><Check size={14} /> Aprovar</button>}</div>
  </div>;
}
