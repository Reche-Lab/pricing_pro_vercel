  ...
  return (
    <section className="border-t border-zinc-800 bg-zinc-950/30">
      ...
      {open ? <div className="grid gap-4 border-t border-zinc-800 p-4">
        ...
        {items.length ? <details className="overflow-hidden rounded-md border border-violet-900/60 bg-violet-950/20">
...
function ArtworkRow({ entry, quoteId, diameter, quantity, busy, onQuantity, onEdit, onApprove }: { entry: ArtworkEntry; quoteId: string; diameter: number | null; quantity: number; busy: string; onQuantity: (quantity: number) => void; onEdit: (entry: ArtworkEntry) => void; onApprove: (entry: ArtworkEntry, status: "approved" | "rejected") => void }) {
  ...
  return <article className="overflow-hidden rounded-md border border-zinc-800 bg-zinc-900/40">
    ...
    <div className="flex flex-wrap items-end justify-between gap-3 border-t border-zinc-800 bg-zinc-950/50 p-3">
      <label className="w-36 shrink-0"><span className="mb-1 block text-xs font-medium text-zinc-400">Cópias desta arte</span><input className="focus-ring h-9 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 text-sm tabular-nums" min="1" type="number" value={quantity} onChange={(event) => onQuantity(Math.max(1, Number(event.target.value) || 1))} /></label>
      <div className="flex min-w-0 flex-1 flex-wrap justify-end gap-2">
        <button className="focus-ring inline-flex h-9 flex-1 basis-[130px] items-center justify-center gap-2 rounded-md border border-zinc-700 px-3 text-xs text-zinc-200 hover:bg-zinc-900 disabled:opacity-50" disabled={!diameter || Boolean(busy)} type="button" onClick={() => onEdit(entry)}><ImageIcon size={14} /> {prepared ? "Reenquadrar" : "Enquadrar"}</button>
        {approved ? <>
          <button className="focus-ring inline-flex h-9 flex-1 basis-[110px] items-center justify-center gap-2 rounded-md bg-emerald-400/15 px-3 text-xs font-medium text-emerald-200 disabled:opacity-50" disabled={Boolean(busy)} type="button" onClick={() => onApprove(entry, "approved")}><Check size={14} /> Atualizar</button>
          <button className="focus-ring grid h-9 w-9 shrink-0 place-items-center rounded-md border border-zinc-700 text-zinc-400 hover:border-red-900 hover:text-red-300" title="Remover aprovação" type="button" onClick={() => onApprove(entry, "rejected")}><X size={14} /></button>
        </> : <button className="focus-ring inline-flex h-9 flex-1 basis-[110px] items-center justify-center gap-2 rounded-md bg-cyan-400 px-3 text-xs font-medium text-cyan-950 disabled:opacity-50" disabled={!prepared || Boolean(busy)} type="button" onClick={() => onApprove(entry, "approved")}><Check size={14} /> Aprovar</button>}
      </div>
    </div>
  </article>;
}
