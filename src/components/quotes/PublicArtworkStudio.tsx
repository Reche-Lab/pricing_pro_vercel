...
export function PublicArtworkStudio({ token, quoteId, items, disabled }: { token: string; quoteId: string; items: QuoteItemRow[]; disabled: boolean }) {
  ...
  return <section className="mt-5 scroll-mt-6 overflow-hidden rounded-lg border border-violet-400/20 bg-zinc-900/70" id="public-artwork-studio">
    ...
    {versionPreview ? <div className="fixed inset-0 z-[110] grid place-items-center bg-black/80 p-3 backdrop-blur-sm" role="dialog" aria-modal="true"><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-lg border border-violet-400/25 bg-zinc-950"><header className="flex items-start justify-between gap-3 border-b border-zinc-800 p-4"><div><p className="font-semibold text-white">Comparar versões</p><p className="mt-1 text-xs text-zinc-400">O retoque está ativo; a original está guardada no histórico.</p></div><button className="grid h-9 w-9 place-items-center rounded-md text-zinc-400 hover:bg-zinc-800" type="button" onClick={() => setVersionPreview(null)}><X size={17} /></button></header><div className="grid gap-4 p-4 sm:grid-cols-2"><PublicVersionImage artwork={versionPreview.active} label="Versão ativa" token={token} /><PublicVersionImage artwork={versionPreview.previous} label="Original" token={token} /></div><footer className="flex flex-wrap justify-end gap-2 border-t border-zinc-800 p-4"><button className="rounded-md border border-zinc-700 px-4 py-2 text-sm text-zinc-300" type="button" onClick={() => setVersionPreview(null)}>Manter retoque</button><button className="rounded-md bg-amber-400 px-4 py-2 text-sm font-semibold text-zinc-950 disabled:opacity-40" disabled={Boolean(busy)} type="button" onClick={restorePreviousVersion}>{busy === `restore-${versionPreview.active.id}` ? "Restaurando..." : "Descartar retoque e restaurar"}</button></footer></div></div> : null}
  </section>;
}
