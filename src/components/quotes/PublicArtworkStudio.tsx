...
export function PublicArtworkStudio({ token, quoteId, items, disabled }: { token: string; quoteId: string; items: QuoteItemRow[]; disabled: boolean }) {
  ...
  const progress = useMemo(() => getPublicArtworkReviewProgress(items.map((candidate) => ({
    ...
    artworks: candidate.artworks?.filter((artwork) => artwork.is_active !== false).map((artwork) => ({ approvalStatus: artwork.approval_status }))
  }))), [items]);
  ...
  async function restorePreviousVersion() {
    if (!versionPreview || !item) return;
    setBusy(`restore-${versionPreview.active.id}`); setMessage("");
    const response = await fetch(`/api/public/quotes/${token}/items/${item.id}/artworks/${versionPreview.active.id}/restore`, { method: "POST" });
    ...
    setVersionPreview(null); setReferenceId(data.restoredArtworkId ?? ""); setMessage("Arte original restaurada. A versão retocada foi descartada."); router.refresh();
  }

  if (!item) return null;
  return <section className="mt-4 min-w-0 scroll-mt-6 overflow-hidden rounded-lg border border-violet-400/20 bg-zinc-900/70 sm:mt-5" id="public-artwork-studio">
    <div className="flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-zinc-800 px-4 py-4 sm:px-5">
      <div className="min-w-0"><p className="inline-flex items-center gap-2 text-sm font-semibold text-white"><Sparkles className="shrink-0 text-violet-300" size={17} /> <span className="break-words">Estúdio de aprovação das artes</span></p><p className="mt-1 text-xs leading-5 text-zinc-400">Envie uma arte pronta, reenquadre, escolha uma versão ou use o assistente criativo.</p></div>
    ...
    <div className="grid min-w-0 gap-4 p-3 sm:gap-5 sm:p-5">
      <div className="flex max-w-full gap-2 overflow-x-auto overscroll-x-contain pb-1">{items.map((candidate, index) => {
        const approved = candidate.artworks?.some((artwork) => artwork.is_active !== false && artwork.approval_status === "approved");
      ...
      {!disabled ? <div className="flex flex-col gap-3 rounded-md border border-cyan-400/25 bg-cyan-400/10 p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="grid w-full shrink-0 gap-2 sm:flex sm:w-auto sm:flex-wrap"><button className="focus-ring inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md border border-cyan-300/50 px-3 py-2 text-center text-sm font-medium text-cyan-100 hover:bg-cyan-300/10 sm:w-auto" type="button" onClick={() => setPdfImportOpen(true)}><FileText size={15} /> PDF com várias artes</button><label className="focus-ring inline-flex min-h-10 w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-cyan-300 px-4 py-2 text-center text-sm font-semibold text-cyan-950 hover:bg-cyan-200 sm:w-auto"><Upload size={15} />{busy === "upload-ready" ? "Enviando..." : itemArtworks.length ? "Enviar nova arte" : "Enviar arte pronta"}<input accept="image/png,image/jpeg,image/webp" className="sr-only" disabled={Boolean(busy)} type="file" onChange={(event) => void upload(event, "ready")} /></label></div>
      </div> : null}
      ...
      {itemArtworks.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{itemArtworks.map((artwork) => {
        ...
        const previous = artwork.parent_artwork_id ? allItemArtworks.find((candidate) => candidate.id === artwork.parent_artwork_id) ?? null : null;
        const original = findOriginalArtwork(artwork, allItemArtworks);
        ...
        return <article className={`overflow-hidden rounded-md border ${approved ? "border-emerald-400/60 bg-emerald-400/5" : referenceId === artwork.id ? "border-violet-400/70 bg-violet-400/5" : "border-zinc-800 bg-zinc-950/60"}`} key={artwork.id}>
          ...
          <div className="grid gap-3 p-3"><div><p className="truncate text-sm font-medium text-white">{artwork.artwork_name || artwork.file_name}</p><p className="mt-1 text-xs text-zinc-500">{prepared ? "Enquadramento preparado" : "Aguardando reenquadramento"}{artwork.source_kind === "openrouter" ? " · criada por IA" : artwork.source_kind === "retouch" ? " · retoque manual" : artwork.source_kind === "pdf_page" ? ` · página ${artwork.source_pdf_page ?? "-"} do PDF` : ""}</p></div>
            <PublicArtworkDownloads artwork={artwork} original={original.id === artwork.id ? null : original} prepared={prepared} token={token} />
            {!disabled ? <div className="grid grid-cols-2 gap-2">{previous ? <button className="focus-ring col-span-2 inline-flex items-center justify-center gap-1.5 rounded-md border border-violet-800/70 px-2 py-2 text-xs text-violet-200 hover:bg-violet-950/50" type="button" onClick={() => setVersionPreview({ active: artwork, previous })}><Eye size={13} /> Ver arte original</button> : null}<button className="focus-ring inline-flex items-center justify-center gap-1.5 rounded-md border border-zinc-700 px-2 py-2 text-xs text-zinc-200 hover:bg-zinc-800" type="button" onClick={() => setRetouching(entry)}><Paintbrush size={13} /> Retocar</button><button className="focus-ring inline-flex items-center justify-center gap-1.5 rounded-md border border-zinc-700 px-2 py-2 text-xs text-zinc-200 hover:bg-zinc-800 disabled:opacity-40" disabled={!geometry} title={geometry ? "Ajustar enquadramento" : "Geometria de impressão não configurada"} type="button" onClick={() => setEditing(entry)}><ImageIcon size={13} /> Reenquadrar</button><button className="focus-ring col-span-2 inline-flex items-center justify-center gap-1.5 rounded-md bg-emerald-400 px-2 py-2 text-xs font-semibold text-emerald-950 disabled:opacity-40" disabled={!prepared || Boolean(busy)} type="button" onClick={() => approve(artwork)}>{busy === `approve-${artwork.id}` ? <Loader2 className="animate-spin" size={13} /> : <Check size={13} />} {approved ? "Aprovada" : "Aprovar esta versão"}</button></div> : null}
          </div>
        </article>;
      ...
      {!disabled ? <details className="overflow-hidden rounded-md border border-violet-900/60 bg-violet-950/20">
        ...
        <div className="grid gap-4 border-t border-violet-900/40 p-4">
          ...
          <div className="grid gap-2 sm:flex sm:flex-wrap"><button className="focus-ring min-h-10 w-full rounded-md border border-violet-700 px-3 py-2 text-xs font-medium text-violet-200 disabled:opacity-50 sm:w-auto" disabled={Boolean(busy)} type="button" onClick={() => runAssistant("suggest")}>{busy === "suggest" ? "Analisando..." : reference ? "Sugerir melhorias" : "Sugerir direção"}</button><button className="focus-ring inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md bg-violet-400 px-3 py-2 text-center text-xs font-semibold text-violet-950 disabled:opacity-50 sm:w-auto" disabled={Boolean(busy) || attemptsRemaining === 0} type="button" onClick={() => runAssistant("generate")}>{busy === "generate" ? <Loader2 className="animate-spin" size={13} /> : <WandSparkles size={13} />}{attemptsRemaining === 0 ? "Limite de gerações atingido" : reference ? "Gerar alteração" : "Criar nova arte"}</button></div>
...
function publicArtworkUrl(token: string, artworkId: string, prepared: boolean, download = false) { const params = new URLSearchParams(); if (prepared) params.set("kind", "prepared"); if (download) params.set("download", "1"); const query = params.toString(); return `/api/public/quotes/${token}/artworks/${artworkId}${query ? `?${query}` : ""}`; }
